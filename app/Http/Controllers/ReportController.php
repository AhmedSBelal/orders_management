...
class ReportController extends Controller
{
    ...
    //     $revenueByStatus = Order::whereBetween('orders.created_at', [$from, $to])  // هنا أضف orders.
    ...
    //     // ============================================
    //     // 1. REVENUE ANALYSIS - FIXED FOR WHOLESALE/RETAIL
    //     // ============================================
        
    //     // First, get active orders (exclude cancelled/returned)
    //     // Assuming you have statuses like 'مكتمل', 'قيد التنفيذ' as active
    //     // And 'ملغي', 'مرتجع' as inactive
    //     $activeStatuses = ['تم الشحن', 'قيد التنفيذ', 'تم التسليم']; // Add your active status names
    ...
    //     // This handles both wholesale and retail correctly
    //     $totalRevenue = DB::table('orders')
    //         ->join('order_product', 'orders.id', '=', 'order_product.order_id')
    //         ->join('order_statuses', 'orders.status_id', '=', 'order_statuses.id')
    //         ->whereBetween('orders.created_at', [$from, $to])
    //         ->whereIn('order_statuses.name', $activeStatuses)
    //         ->select(DB::raw('SUM(order_product.quantity * order_product.price) as total'))
    //         ->value('total') ?? 0;

    //     // Alternatively, if you want to use total_price field
    //     // We need to ensure total_price is correctly saved
    //     $totalRevenueSimple = $ordersQuery->sum(DB::raw('COALESCE(total_price_after_discount, total_price)'));

    //     // For now, let's use the pivot calculation as it's more accurate
    ...
    //             'order_statuses.name',
    //             DB::raw('COUNT(*) as orders_count'),
    //             DB::raw('SUM(COALESCE(orders.total_price_after_discount, orders.total_price)) as total')
    //         )
    //         ->groupBy('order_statuses.name')
    //         ->get();
    ...
    //     // ============================================
    //     // 3. PRODUCT PROFITABILITY ANALYSIS - FIXED
    //     // ============================================
    //     $productProfitability = DB::table('order_product')
    //         ->join('orders', 'order_product.order_id', '=', 'orders.id')
    //         ->join('order_statuses', 'orders.status_id', '=', 'order_statuses.id')
    ...
    //             'products.id',
    //             'products.name',
    //             'products.price as retail_price', // السعر القطاعي
    //             'products.wholesale_price', // سعر الجملة
    //             'products.cost',
    //             DB::raw('SUM(order_product.quantity) as total_quantity'),
    //             DB::raw('SUM(order_product.price * order_product.quantity) as total_revenue'), // استخدام السعر الفعلي من pivot
    //             DB::raw('SUM(products.cost * order_product.quantity) as total_cost'),
    ...
    //         // Calculate average selling price (mix of wholesale and retail)
    ...
    //             DB::raw('SUM(COALESCE(total_price_after_discount, total_price)) as total_revenue'),
    //             DB::raw('AVG(COALESCE(total_price_after_discount, total_price)) as avg_order_value'),
    //             DB::raw('SUM(CASE WHEN is_wholesale = 1 THEN 1 ELSE 0 END) as wholesale_orders'),
    //             DB::raw('SUM(CASE WHEN is_wholesale = 0 THEN 1 ELSE 0 END) as retail_orders')
    //         )
    //         ->groupBy('come_from')
    //         ->get();
    ...
    //     $wholesaleVsRetail = Order::whereBetween('created_at', [$from, $to])
    //         ->whereHas('status', function ($query) use ($activeStatuses) {
    //             $query->whereIn('name', $activeStatuses);
    //         })
    //         ->select(
    //             DB::raw('SUM(CASE WHEN is_wholesale = 1 THEN 1 ELSE 0 END) as wholesale_orders'),
    //             DB::raw('SUM(CASE WHEN is_wholesale = 0 THEN 1 ELSE 0 END) as retail_orders'),
    //             DB::raw('SUM(CASE WHEN is_wholesale = 1 THEN COALESCE(total_price_after_discount, total_price) ELSE 0 END) as wholesale_revenue'),
    //             DB::raw('SUM(CASE WHEN is_wholesale = 0 THEN COALESCE(total_price_after_discount, total_price) ELSE 0 END) as retail_revenue'),
    //             DB::raw('AVG(CASE WHEN is_wholesale = 1 THEN COALESCE(total_price_after_discount, total_price) ELSE NULL END) as avg_wholesale_order'),
    //             DB::raw('AVG(CASE WHEN is_wholesale = 0 THEN COALESCE(total_price_after_discount, total_price) ELSE NULL END) as avg_retail_order')
    //         )
    //         ->first();
